...
import { MODULE_NS as READER_NS, SET_TEXT_SIZE } from "@/store/constants";
...
describe("TextSizeWidget.vue", () => {
  ...
  it("sets the correct default data", () => {
    ...
    expect(store.state[READER_NS].readerTextSize).toBe("md");
  });
  ...
  it('updates the "readerTextSize" appropriately', () => {
    ...
    expect(store.state[READER_NS].readerTextSize).toBe("md");
    store.dispatch(`${READER_NS}/${SET_TEXT_SIZE}`, { size: "sm" });
    expect(wrapper.html()).toContain(
      '<div class="text-size-widget"><textsize-stub size="xs" value="sm"></textsize-stub>'
    );
    expect(store.state[READER_NS].readerTextSize).toBe("sm");
    store.dispatch(`${READER_NS}/${SET_TEXT_SIZE}`, { size: "lg" });
    expect(wrapper.html()).toContain(
      '<div class="text-size-widget"><textsize-stub size="xs" value="lg"></textsize-stub>'
    );
    expect(store.state[READER_NS].readerTextSize).toBe("lg");
    store.dispatch(`${READER_NS}/${SET_TEXT_SIZE}`, { size: "xl" });
    expect(wrapper.html()).toContain(
      '<div class="text-size-widget"><textsize-stub size="xs" value="xl"></textsize-stub>'
    );
    expect(store.state[READER_NS].readerTextSize).toBe("xl");
    store.dispatch(`${READER_NS}/${SET_TEXT_SIZE}`, { size: "md" });
    expect(wrapper.html()).toContain(
      '<div class="text-size-widget"><textsize-stub size="xs" value="md"></textsize-stub>'
    );
    expect(store.state[READER_NS].readerTextSize).toBe("md");
    ...
    expect(store.state[READER_NS].readerTextSize).toBe("xs");
    expect(store.state[READER_NS].readerTextWidth).toBe("normal");
